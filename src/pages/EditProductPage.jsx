...
function EditProductPage({ productToEdit, currentUser, onBackToDashboard, onProductUpdated }) {
  ...
  const [formData, setFormData] = useState({
    ...
    expiryDate: formatInitialDate(productToEdit?.expiryDate),
  ...
  useEffect(() => {
    if (!productToEdit && onBackToDashboard) {
      onBackToDashboard()
    }
  }, [productToEdit, onBackToDashboard])
  ...
  const handleBarcodeDetected = (code) => {
    ...
    setSuccessMsg(`UPC Barcode updated to ${code}`)
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await updateProduct(productToEdit._id, {
    ...
    if (response.success) {
      ...
      setErrorMsg(response.message || 'Failed to update product. Please check server logs.')
    }
  }
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
          ...
          <div className="bg-slate-900 text-white p-6 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-extrabold text-white tracking-tight">Edit Product Details</h1>
                <p className="text-xs text-slate-400 mt-0.5">
                  Update item information, UPC barcode, or expiry date for "{productToEdit.title}"
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-500 font-medium">Barcode / Camera Scan</span>
              </div>
